function Invoke-Control2225 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 2.2.25: Current list of Groups and User Accounts granted the Deny log on locally right"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 2.2.25"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 2.2.25: Current list of Groups and User Accounts granted the Deny log on locally right"
        try {
            ...
            $cmdOutput = "Executed remediation step for Control ID 2.2.25"
            Write-Host $cmdOutput
            Write-Log $cmdOutput
        } catch {
            Write-Log "ERROR applying remediation for Control ID 2.2.25: $_"
}
}
