function Invoke-Control1877 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 18.7.7: Status of the Configure RPC over TCP port setting"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 18.7.7"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 18.7.7: Status of the Configure RPC over TCP port setting"
        try {
            ...
            Write-Log "ERROR applying remediation for Control ID 18.7.7: $_"
}
}
