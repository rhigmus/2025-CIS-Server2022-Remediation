function Invoke-Control1872 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 18.7.2: Status of the Configure Redirection Guard setting"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 18.7.2"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 18.7.2: Configure Redirection Guard"
        try {
            ...
            Write-Log "ERROR applying remediation for Control ID 18.7.2: $_"
}
}
