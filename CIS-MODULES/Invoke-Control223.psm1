function Invoke-Control223 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 2.2.3: Current list of Groups and User Accounts granted the Access this computer from the network right"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 2.2.3"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 2.2.3: Current list of Groups and User Accounts granted the Access this computer from the network right"
        try {
            ...
            Write-Log "ERROR applying remediation for Control ID 2.2.3: $_"
}
}
