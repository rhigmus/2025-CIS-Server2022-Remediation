function Invoke-Control228 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 2.2.8: Current list of Groups and User Accounts granted the Allow log on locally (SeInteractiveLogonRight) right"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 2.2.8"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 2.2.8: Current list of Groups and User Accounts granted the Allow log on locally (SeInteractiveLogonRight) right"
        try {
            ...
            $cmdOutput = "Executed remediation step for Control ID 2.2.8"
            Write-Host $cmdOutput
            Write-Log $cmdOutput
        } catch {
            Write-Log "ERROR applying remediation for Control ID 2.2.8: $_"
}
}
