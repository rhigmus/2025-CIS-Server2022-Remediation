function Invoke-Control186141 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 18.6.14.1: Status of the Hardened UNC Paths setting for Sysvol"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 18.6.14.1"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 18.6.14.1: Status of the Hardened UNC Paths setting for Sysvol"
        try {
            ...
            Write-Log "ERROR applying remediation for Control ID 18.6.14.1: $_"
}
}
