function Invoke-Control2374 {
    param([bool]$Apply = $false)

    Write-Host "`nControl ID 2.3.7.4: Status of the contents of the login banner (Windows/Unix/Linux)"
    if (-not $Apply) {
        $confirm = Read-Host "Apply this remediation? (y/n)"
        if ($confirm -ne "y") {
            Write-Log "User skipped remediation for Control ID 2.3.7.4"
            return
        }
    }
        Write-Log "User approved remediation for Control ID 2.3.7.4: Status of the contents of the login banner (Windows/Unix/Linux)"
        try {
            ...
            Write-Log "ERROR applying remediation for Control ID 2.3.7.4: $_"
}
}
